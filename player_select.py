...
def compute_tables(GAMES_TO_CHECK, prints_on = False):
    ...
    if prints_on:
        ...
        print(df_attack.sort_values(by='Atk c'))

        print("")
        print("---- Defense Table ----")
        print(df_defense.sort_values(by='Def c'))
    ...
    df_attack_multi = df_attack[['HomeTeam', 'Atk c']]
    df_defense_multi = df_defense[['HomeTeam', 'Def c']]
    ...
    df_reg['clean_90'] = (df_reg['clean_sheets'] / df_reg['minutes']) * 90
    ...
    df_reg = df_reg.dropna(subset=[
        'pts_90', 'threat_90', 'creativity_90', 'team_name','cbit_90','cbirt_90','clean_90'
    ])
    ...
    if prints_on:
        print("\n" + "="*30 + " DEF Model " + "="*30)
    if not df_def.empty:
        def_formula = 'pts_90 ~ clean_90   +I(cbit_90**2)+ creativity_90 + threat_90'
    ...
    if prints_on:
        print("\n" + "="*30 + " MID Model " + "="*30)
    if not df_mid.empty:
        mid_formula = 'pts_90 ~ clean_90 + creativity_90 + I(cbirt_90**2) + threat_90'
    ...
    df_final_predictions['fx_predicted_pts_90'] = np.where(
        ...
        df_final_predictions['predicted_pts_90'] * df_final_predictions['Atk c'], # Value if True
        df_final_predictions['predicted_pts_90'] * df_final_predictions['Def c']  # Value if False (GKP or DEF)
    )


    #form blend (Optional)
    df_final_predictions['fx_predicted_pts_90'] = df_final_predictions['fx_predicted_pts_90'] * (1 - form_weight) + (df_final_predictions['form'] * form_weight)    
    ...
    if prints_on:
        ...
        # Display the most useful columns, sorted by the new prediction
        print(df_final_predictions[df_final_predictions['position']=="MID"].sort_values(by='fx_predicted_pts_90', ascending=False)[cols_to_display].head(100))
...
def find_all_transfer_opportunities(df_team_list, df_1, df_5, bank=0.0, top_n=5):
    ...
    for idx, team_player_row in df_team_list.iterrows():
        ...
        # Get current player's info from df_1 and df_5
        current_player_info = df_1[df_1['web_name'] == team_player_name]

        current_player_info_5 = df_5[df_5['web_name'] == team_player_name]
        ...
        current_pts_df5 = current_player_info_5['fx_predicted_pts_90'].values[0]
        ...
        for idx_5, potential_player in same_position_players.iterrows():
            ...
            # Skip if the potential player is already in your team (any position)
            if potential_name in df_team_list['web_name'].values:
                continue
            ...
            if potential_pts_df5 > current_pts_df5 and potential_pts_df1 > current_pts_df1:
                pts_diff = potential_pts_df5 - current_pts_df5
...
df_transfers = find_all_transfer_opportunities(df_team_list, df_1, df_5,bank =1.8, top_n=50)
